                              ...
                              char * datosParaPC, int * ptrError)
{
  ...
  char datosParaPMAC[6] = {'&', '1', 'B', '9', 'R', '\0'};
  ...
  if (escribirDPRAM(descriptorFich, DIR_MP9_TIEMPO_CURVAS_TRAMO1, (char *)comandosT, NBYTES))
  {
    return -1;
  }
  if (escribirDPRAM(descriptorFich, DIR_MP9_TIEMPO_CURVAS_TRAMO2, (char *)(comandosT + 1), NBYTES))
  {
    return -1;
  }
  if (escribirDPRAM(descriptorFich, DIR_MP9_DESPLAZ_CON_SIGNO_TRAMO1, (char *)comandosPDer, NBYTES))
  {
    return -1;
  }
  if (escribirDPRAM(descriptorFich, DIR_MP9_DESPLAZ_CON_SIGNO_TRAMO2, (char *)(comandosPDer + 1), NBYTES))
  {
    return -1;
  }
  if (escribirDPRAM(descriptorFich, DIR_MP9_VEL_FINAL_TRAMO1, (char *)comandoVDer, NBYTES))
  {
    return -1;
  }
  ...
  //return -1;
  //}
  if (leerDPRAM(descriptorFich, DIR_MP9_TIEMPO_CURVAS_TRAMO1, (char *)&tiempoMediaCurvaSTramo1, NBYTES))
  {
    return -1;
  }
  printf("tiempoMediaCurvaSTramo1: %d ms\n", tiempoMediaCurvaSTramo1);

  if (leerDPRAM(descriptorFich, DIR_MP9_TIEMPO_CURVAS_TRAMO2, (char *)&tiempoMediaCurvaSTramo2, NBYTES))
  {
    return -1;
  }
  printf("tiempoMediaCurvaSTramo2: %d ms\n", tiempoMediaCurvaSTramo2);

  if (leerDPRAM(descriptorFich, DIR_MP9_DESPLAZ_CON_SIGNO_TRAMO1, (char *)&desplazConSignoTramo1, NBYTES))
  {
    return -1;
  }
  printf("desplazConSignoTramo1: %f cm\n", desplazConSignoTramo1);

  if (leerDPRAM(descriptorFich, DIR_MP9_DESPLAZ_CON_SIGNO_TRAMO2, (char *)&desplazConSignoTramo2, NBYTES))
  {
    return -1;
  }
  printf("desplazConSignoTramo2: %f cm\n", desplazConSignoTramo2);

  if (leerDPRAM(descriptorFich, DIR_MP9_VEL_FINAL_TRAMO1, (char *)&velFinalTramo1, NBYTES))
  {
    return -1;
  }
  printf("velFinalTramo1: %f cm/s\n", velFinalTramo1);
...
// 'DIR_FLAG_ACTUALIZAR_ODOMETRIA' el valor 1, usando 4 bytes.
  ...
  do
  {
    if (leerDPRAM(descriptorDPRAM, DIR_FLAG_ODOMETRIA_ACTUALIZADA, (char *)&flagOdometriaActualizada, NBYTES) == -1)
    {
      return -1;
    }
  ...
  memset(x_mbase_modom, '\0', sizeof(float));
  memset(y_mbase_modom, '\0', sizeof(float));
  memset(t_mbase_modom, '\0', sizeof(float));
// El pc ya puede leer de la DPRAM el radio con signo.
  if (leerDPRAM(descriptorDPRAM, DIR_X_ROBOT_ODOM, (char *)x_mbase_modom, NBYTES) == -1)
  {
    return -1;
  }
  if (leerDPRAM(descriptorDPRAM, DIR_Y_ROBOT_ODOM, (char *)y_mbase_modom, NBYTES) == -1)
  {
    return -1;
  }
  if (leerDPRAM(descriptorDPRAM, DIR_T_ROBOT_ODOM, (char *)t_mbase_modom, NBYTES) == -1)
  {
    return -1;
  }
...
// robot en el SRO.
  ...
  if (escribirDPRAM(descriptorDPRAM, DIR_FLAG_ODOMETRIA_ACTUALIZADA, (char *)&flagOdometriaActualizada, NBYTES) == -1)
  {
    // La funcion finaliza sin exito.
    return -1;
  }
  *x_mbase_modom = 0.01 * (*x_mbase_modom); // En m.
  *y_mbase_modom = 0.01 * (*y_mbase_modom); // En m.
...
int resetearOdometriaPMAC(int descriptorFich)
{
  ...
  if (escribirDPRAM(descriptorFich, DIR_RESET_ODOMETRIA, (char *)&flagReset, NBYTES))
  {
    return -1;
  }

  // Esperar a que haya terminado de resetear la PMAC las variables adecuadas en la DPRAM.
  do
  {
    if (leerDPRAM(descriptorFich, DIR_RESET_ODOMETRIA, (char *)&flagReset, NBYTES))
    {
      return -1;
    }
  } while (flagReset);
