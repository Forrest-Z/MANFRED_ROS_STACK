...
public:
	...
	inline T getCellValue(int x, int y) const {
		 return *grid[x][y];
	};
	...
	inline bool WorldToGrid(double worldX, double worldY, int& gridX, int& gridY){
		...
		// sVilches: Added "&& (gridX >= 0) && (gridY >= 0)"
		if ((gridX < width) && (gridY < height) && (gridX >= 0) && (gridY >= 0)) return true;
		else return false;
	}
	...
	inline bool isThereCellValueInRay(Point2o point, double angle, T value, double maxdistance, RayPoint& rayPoint){
		return isThereCellValueInRay(point.x, point.y, point.theta.getValue()+angle, value, maxdistance, rayPoint);
	}
