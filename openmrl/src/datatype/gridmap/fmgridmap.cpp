...
IMPLEMENT_MRL_OBJECT(FMGridMap)

void FMGridMap::drawGL2D() const{
	 GridMap<FMPixelData>::drawGL2D();
}
...
void FMGridMap::buildFromFMGridMapStar(FMGridMap* fmmap, int targetX, int targetY, int initX, int initY, bool directional){
		...
		for (int aux=0;aux<2;aux++){
			for (int l=-1;l<2;l++){
				...
				if (pData->status != FMPixelData::FROZEN){
					...
					double optimal_cost;
					//FIXME The /5 is regulated by the thumb rule
					//When we are in the saturated zone we should not use the real optimal cost
...
void FMGridMap::buildFromFMGridMap(FMGridMap* fmmap, int pixelX, int pixelY, int initX, int initY, bool directional){
	//max = safe_distance / max_speed;
	max=0;
	//precomputedDists=NULL;
	this->free();
		...
		for (int aux=0;aux<2;aux++){
			for (int l=-1;l<2;l++){
				...
				if (pData->status != FMPixelData::FROZEN){
					 ...
					 //Normal FM2
					 if (!directional){
						  speed = fmmap->getCellValue(x,y).d;
					 }else{ //FM2 that takes in consideration direction
						  double speed_inc = fmmap->getCellValue(x,y).d
									 -fmmap->getCellValue(current->x, current->y).d;
						  if (speed_inc >= 0){ //we are going away from obstacles
								speed = max_speed;
						  }else{ //we are approaching an obstacle, take usual speed
								speed = fmmap->getCellValue(x,y).d;
						  }
					 }
...
void FMGridMap::buildFromFMGridMap(FMGridMap* fmmap, int pixelX, int pixelY, bool directional){
	//max = safe_distance / max_speed;
	max=0;
	//precomputedDists=NULL;
	this->free();
