...
{
  if ((isExtensionOfFile(filename, "jpg")) || (isExtensionOfFile(filename, "jpeg")))
    return load_jpg(filename);
  else if (isExtensionOfFile(filename, "bmp"))
    return load_bmp(filename);
  else
    cerr << "Extension not known" << endl;
  return false;
}

bool Image::load_jpg(string filename)
{
  cerr << "JPEG images cannot be loaded on the current implementation" << endl;
  return false;
}
...
bool Image::load_bmp(string filename)
{
  ...
  {
    cerr << filename << " is not a bitmap file" << endl;
    fclose(file);
    return false;
  }
  ...
  int bmpImageSize = header[0x22] + (header[0x23] << 8) + (header[0x24] << 16) + (header[0x25] << 24);
  ...
  if (biBiLayers != 1)
  {
    cerr << "biBiLayers is not 1. It is" << biBiLayers << "." << endl;
    fclose(file);
    return false;
  }
  ...
  if (biBitCount != 24)
  {
    cerr << "It is not an RBG image. It is " << biBitCount << "." << endl;
    fclose(file);
    //file.close();
    return false;
  }

  unsigned int num_bytes_row = bmpWidth * 3 * sizeof(unsigned char);
  // printf("num_bytes_row: %d.\n", num_bytes_row);
  /* calculate the size of the image in bytes */
  long int biSizeImage = num_bytes_row * bmpHeight;

  unsigned short num_bytes_padding = (bmpImageSize - biSizeImage) / bmpHeight;
  // printf("num_bytes_padding: %d.\n", num_bytes_padding);
  ...
  // ptr_buffer points to the right place at buffer: the position at which begins the last row of the image.
  BYTE *ptr_buffer = buffer + ((bmpHeight - 1) * num_bytes_row);

  size_t result = 0;

  for (int i = 0; i < bmpHeight; i++)
  {
    if (!fread(ptr_buffer, num_bytes_row, 1, file))
    {
      cerr << "Unable to parse the bitmap data" << endl;
      fclose(file);
      return false;
    }
    ptr_buffer -= num_bytes_row;
    // skip bytes of padding at the end of the actual row.
    if (fseek(file, num_bytes_padding, SEEK_CUR))
    {
      cerr << "Unable to parse the bitmap data 2" << endl;
      fclose(file);
      return false;
    }
  }
...
bool Image::save_jpg(string filename, const WORD quality)
{
  ...
  return true;
#else
  cerr << "LibJPEG is not installed on your system. You cannot save in jpeg format" << endl;
  return false;
#endif
