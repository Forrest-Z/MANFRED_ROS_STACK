...
public:
	 ...
	 */ 
	inline void computeFM2(FMGridMap* fmmap, int pixelX, int pixelY){
		max=0;
		heap.clear() ;

		freeall(); //free al allocated memory
		
		buildFromFMGridMap(fmmap, pixelX, pixelY); // fill the fmgridmap
	}
	 ...
	 */
	inline void computeFM2(FMGridMap* fmmap, int targetX, int targetY, int initX, int initY, double max_speed=1, bool star = true){
		max=0;
		heap.clear() ;
		...
		if(star){
			buildFromFMGridMapStar(fmmap, targetX, targetY, initX, initY);
		}else{
			buildFromFMGridMap(fmmap, targetX, targetY, initX, initY);
		}
	}
	...
	void buildFromFMGridMap(FMGridMap* fmmap, int pixelX, int pixelY);
	void buildFromFMGridMap(FMGridMap* fmmap, int targetX, int targetY, int initX, int initY);
	void buildFromFMGridMapStar(FMGridMap* fmmap, int targetX, int targetY, int initX, int initY);
