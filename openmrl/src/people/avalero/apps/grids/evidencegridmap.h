 ...
 * PURPOSE.
 **********************************************************************/
#ifndef __MRCORE__EVIDENCEGRIDMAP_H
#define __MRCORE__EVIDENCEGRIDMAP_H
...
class EvidenceGridMap : public GridMap<unsigned char>, public GLObject {

	///Text stream serializers (files)
	friend ostream& operator << (ostream& os,const EvidenceGridMap& gridMap);

	DECLARE_MR_OBJECT(EvidenceGridMap)

public:
	enum Elements{ OBSTACLE=0, FREE=255, UNKNOWN=127 };

	EvidenceGridMap(){width=height=resolution=0; buffer=NULL;grid=NULL;}
	/**
	 ...
	 */
	EvidenceGridMap(double real_width, double real_height, double resolution, unsigned char default_value = UNKNOWN, double mapX=0, double mapY=0);
	~EvidenceGridMap();
