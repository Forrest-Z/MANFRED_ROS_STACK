...
IMPLEMENT_MR_OBJECT(EvidenceGridMap)


EvidenceGridMap::EvidenceGridMap(double real_width, double real_height, double resolution, unsigned char default_value, double mapX, double mapY)
...
EvidenceGridMap::~EvidenceGridMap(){
	for (int i=0;i<256;i++) delete [] rgb_colors[i];
	delete [] rgb_colors;
}

Image* EvidenceGridMap::convertToImage(int w) const{
	Image *img =new Image(this->width, this->height);
	for (int x=0; x < this->width; x++){
		for (int y=0; y < height; y++){
			unsigned char* color = rgb_colors[ *grid[x][y] ];
...
void EvidenceGridMap::loadFromImage(Image* img, double real_width, double resolution, double mapX, double mapY){
	...
	for (int i=0; i<w; i++){
		for (int j=0; j<h; j++){
			for (int k=0; k<256; k++)
			{
				if(  (abs( img_buf[i][j][0] - rgb_colors[k][0] ) < 10 )
				  && (abs( img_buf[i][j][1] - rgb_colors[k][1] ) < 10 )
				  && (abs( img_buf[i][j][2] - rgb_colors[k][2] ) < 10 ) ){
					  *grid[i][height-j-1]=k;
				  }
			}
		}
...
void EvidenceGridMap::writeToStream(Stream& os){
	GridMap::writeToStream(os);
}

void EvidenceGridMap::readFromStream(Stream& is){
	GridMap::readFromStream(is);
}

void EvidenceGridMap::drawGL(){
