...
public: 
	...
	struct RayPoint{
		RayPoint(){;}
		RayPoint(int px, int py, double wx, double wy,T v){pixelX=px;pixelY=py;x=wx;y=wy;value=v;}
		int pixelX, pixelY;
		double x,y;
		T value;
	};
...
public:
	...
	inline T getCellValue(int x, int y){ return *grid[x][y]; }
	 ...
	 */
	inline bool getSafeCellValue(int x, int y, T& value){
		if (x<width && y <height){
	...
	inline bool isThereCellValueInRay(Point2o point, T value, double maxdistance, RayPoint& rayPoint){
		return isThereCellValueInRay(point.x, point.y, point.theta, value, maxdistance, rayPoint);
	}
	...
	inline bool isThereCellValueSetInRay(Point2o point, set<T> valueSet, double maxdistance, RayPoint& rayPoint){
		return isThereCellValueInRay(point.x, point.y, point.theta, valueSet, maxdistance, rayPoint);
	}
	...
	inline Ray getRay(Point2o point, double distance){
		return getRay(point.x, point.y, point.theta, distance);
	}
