...
void OnMouseClick(int b,int state, int x,int y)
{
	...
	if (down) {
		...
		robot.map = gridmap;
		robot.sensors.push_back( new RangeSensor("IR_L0", Pose(0.1,0,0), 0.15, 0, 1));
		robot.sensors.push_back( new RangeSensor("IR_L1", Pose(0,0.1,M_PI_2), 0.15, 0, 1));
		robot.sensors.push_back( new RangeSensor("IR_L2", Pose(-0.1,0,M_PI), 0.15, 0, 1));
		robot.sensors.push_back( new RangeSensor("IR_L1", Pose(0,-0.1,-M_PI_2), 0.15, 0, 1));	
		//************  END ROBOT   ****************//
		robot.setPose(rx,ry,angle);
		robot.updateRangeData();
		if (scene.count("z1_point")>0) delete scene["z1_point"];
		scene["z1_point"] = p;
		if (scene.count("z2_range")>0) delete scene["z2_range"];
		scene["z2_range"] = robot.rangeData;
	}
	if (scene.size()==0) return;
	glutPostRedisplay();
}
