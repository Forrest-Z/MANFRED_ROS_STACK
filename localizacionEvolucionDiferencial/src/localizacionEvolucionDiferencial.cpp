...
LocED::LocED(const unsigned int& d, const unsigned int& st, const float& cr, const float& f, const float& t_c,
    ...
    numero_componentes_(d), estrategia_mutacion_(st), factor_cruce_(cr), factor_atenuacion_(f), dist_traslacion_C_(t_c), dist_traslacion_M_(
        0), dist_convergencia_C_(0), dist_convergencia_M_(dist_conv_m), lado_celda_M_(0), bandera_loc_global_exito_(0), num_elem_pob_norm_(
        num_elem_pob_norm), num_elem_pob_conv_(num_elem_pob_conv), num_elem_pob_act_(num_elem_pob_norm), num_iter_norm_(
        num_iter_norm), num_iter_conv_(num_iter_conv), num_iter_act_(num_iter_norm), mapa_recibido_(0), mapa_a_(
        nomb_fich_a),
        ...
        aptitudUbEstSrg_(0), numLocSinExito_(0), alcance_laser_M_(alcance_laser_m), alcance_laser_C_(0), cAnguloMax(
            scRadGrad * angulo_max), cResAngular(scRadGrad * res_angular), cNumMedLaser(numMedLaser), cDist3SigmaM(
            dist_3sigma_m),
        ...
        cNumMedLaserNorm(2 * ((unsigned int)(angulo_max / (coef_incr * res_angular))) + 1), cNumMedLaserConv(
            ...
            0), coef_angulo_(0), intervaloTiempoSegLocSinExito_(0)
{

  // cAptitudLimSup = 2 * ((2 * dist_3sigma_m) * num_med_laser_act_);
  // double dist_3sigma_c = dist_3sigma_m / ladoCeldaM_;
  ...
  {
    ang_[i] = angulo;
    // cout << i << " " << ang_[i] << " " << scGradRad * ang_[i] << "\n";
    angulo += cResAngular;
  }

  cout << "numero_componentes_: " << numero_componentes_ << endl;
  cout << "estrategia_mutacion_: " << estrategia_mutacion_ << endl;
  cout << "factor_cruce_: " << factor_cruce_ << endl;
  cout << "factor_atenuacion_: " << factor_atenuacion_ << endl;
  cout << "lado_celda_M_: " << lado_celda_M_ << endl;
  cout << "dist_traslacion_C_: " << dist_traslacion_C_ << endl;
  cout << "dist_traslacion_M_: " << dist_traslacion_M_ << endl;
  cout << "dist_convergencia_C_: " << dist_convergencia_C_ << endl;
  cout << "dist_convergencia_M_: " << dist_convergencia_M_ << endl;
  cout << "coef_hipotenusa_: " << coef_hipotenusa_ << endl;
  cout << "coef_angulo_: " << coef_angulo_ << endl;
  cout << "umbral_ocupacion_: " << umbral_ocupacion_ << endl;
  cout << "umbral_no_ocupacion_: " << umbral_no_ocupacion_ << endl;
  cout << "bandera_loc_global_exito_: " << bandera_loc_global_exito_ << endl;
  cout << "num_elem_pob_norm_: " << num_elem_pob_norm_ << endl;
  cout << "num_elem_pob_conv_: " << num_elem_pob_conv_ << endl;
  cout << "num_elem_pob_act_: " << num_elem_pob_act_ << endl;
  cout << "num_iter_norm_: " << num_iter_norm_ << endl;
  cout << "num_iter_conv_: " << num_iter_conv_ << endl;
  cout << "num_iter_act_: " << num_iter_act_ << endl;
  cout << "Fichero mapa a: " << mapa_a_.getNombreFich() << endl;

#ifdef DOS_MAPAS

  cout << "Fichero mapa b: " << mapa_b_.getNombreFich() << endl;

#endif

  cout << "Numero filas mapa A/B: " << mapa_a_.getFilas() << endl;
  cout << "Numero columnas mapa A/B: " << mapa_a_.getColumnas() << endl;

  cout << "x_est_busq_g: " << ubEstSrg_.getX() << endl;
  cout << "y_est_busq_g: " << ubEstSrg_.getY() << endl;
  cout << "t_est_busq_g: " << scGradRad * ubEstSrg_.getTheta() << endl;
  cout << "aptitudUbEstSrg_: " << aptitudUbEstSrg_ << endl;
  cout << "Tamanio poblacion_: " << poblacion_.size() << endl;
  cout << "Tamanio pobInicioLoc_: " << pobInicioLoc_.size() << endl;
  cout << "Tamanio coste_pob_: " << coste_pob_.size() << endl;
  cout << "cAptitudLimSup: " << cAptitudLimSup << endl;
  cout << "alcance_laser_M_: " << alcance_laser_M_ << endl;
  cout << "alcance_laser_C_: " << alcance_laser_C_ << endl;
  cout << "cAnguloMax: " << cAnguloMax << " rad " << endl;
  cout << "cAnguloMax: " << scGradRad * cAnguloMax << " grad " << endl;
  cout << "cResAngular: " << cResAngular << " rad " << endl;
  cout << "cResAngular: " << scGradRad * cResAngular << " grad " << endl;
  cout << "cNumMedLaser: " << cNumMedLaser << endl;
  //cout << "cCoefNumMedLaser: %d.\n", cCoefNumMedLaser);
  cout << "cNumMedLaserNorm: " << cNumMedLaserNorm << endl;
  cout << "cNumMedLaserConv: " << cNumMedLaserConv << endl;
  cout << "num_med_laser_act_: " << num_med_laser_act_ << endl;
}
                                ...
                                const std::vector<double>& distLaserAnterior)
{
  ...
  // Aumento del 25% del tamaño del lado: (1+0'25)*(0'5*dist_convergencia_C_) = 1'25*0'5*dist_convergencia_C_ = 0'625*dist_convergencia_C_;
  //float mitad_lado_reg_conv = 0.5 * dist_convergencia_C_;
  ...
  // interior de una zona cuadrada de 'dist_convergencia_C_' celdas de lado.
  ...
  // Ubicacion estimada buena y poblacion concentrada en un cuadrado de 'dist_convergencia_C_' celdas de lado:
  // EL ALGORITMO ALCANZA LA CONVERGENCIA.
  if (aptitudUbEstSrg_ < cAptitudLimSup && incrX < dist_convergencia_C_ && incrY < dist_convergencia_C_)
  {
    ...
    poblacion_.resize(num_elem_pob_conv_);
  ...
  // Ubicacion estimada no buena y poblacion no concentrada en un cuadrado de 'dist_convergencia_C_' celdas de lado.
  // Ubicacion estimada si buena y poblacion no concentrada en un cuadrado de 'dist_convergencia_C_' celdas de lado.
  else
  {
    cout << endl;
    cout << "\x1B[31;1m";
...
// El algoritmo alcanzo la convergencia en un paso anterior.
    if (bandera_loc_global_exito_)
    {
      ...
      else
      {
        ...
        double ladoRegInt = intervaloTiempoSegLocSinExito_ * 20; // 20 CM/S VEL LIN MAX ROBOT
        ...
        ub_max_regint_g_.setXYTheta(mapa_a_.getColumnas() - scRound, mapa_a_.getFilas() - scRound, dosPi);
      }
    }
    else
    {
...
// A PARTIR DE AQUI LA UBICACION SELECCIONADA ESTA DENTRO DE LA REGION DE INTERES.
  int c_x_g = 0;
  int f_y_g = 0;
  double prob_ocupacion_a = 0.0;
  roundLoc(x_g, y_g, c_x_g, f_y_g);
  prob_ocupacion_a = mapa_a_.obtenerProbabilidadOcupacion(c_x_g, f_y_g);
  if (prob_ocupacion_a >= umbral_ocupacion_)
  {
    //cout << "Individuo representa una ubicacion ocupada" << endl;
    return DBL_MAX ;
  }
...
double LocED::aptitudIndividuoConConvergencia(const Ubicacion & ub_g, const std::vector<double> & dist_laser)
{
  ...
  for (unsigned int i = 0; i < num_med_laser_act_; ++i)
  {
    ...
    // cout << "\n";
    // cout << "T_EX_G: " << scGradRad * t_ex_g << ".\n";
    // cout << "INC_X_EX_G: " << incr_x_ex_g << ".\n";
    // cout << "INC_Y_EX_G: " << incr_y_ex_g << ".\n";
  ...
  // cout << "\n";
  // cout << "AptitudIndividuo -- aptitud: " << aptitud << "\n";
  ...
  aptitud += (coef_hipotenusa_ * (lado_celda_M_ * hypot(incr_x_g, incr_y_g))) + (coef_angulo_ * fabs(incr_t_g));
...
// for (i = 0; i < num_med_laser_act_; ++i) {
//    cout << i << " -- " << ind_med_laser_[i] << " -- " << scGradRad * ang_[ind_med_laser_[i]] << ".\n";
//}
...
// A PARTIR DE AQUI LA UBICACION SELECCIONADA ESTA DENTRO DE LA REGION DE INTERES.
  ...
  int f_y_g = 0;
  double prob_ocupacion_a = 0.0;
  double prob_ocupacion_b = 0.0;

// Comprobar si la posicion seleccionada esta ocupada.
  roundLoc(x_g, y_g, c_x_g, f_y_g);
  prob_ocupacion_a = mapa_a_.obtenerProbabilidadOcupacion(c_x_g, f_y_g);
  prob_ocupacion_b = mapa_b_.obtenerProbabilidadOcupacion(c_x_g, f_y_g);

// Ubicacion seleccionada ocupada en ambos mapas.
  if (prob_ocupacion_a >= umbral_ocupacion_ && prob_ocupacion_b >= umbral_ocupacion_)
  {
    return DBL_MAX;
  }
...
// Coste de la ubicacion seleccionada.
  ...
  for (i = 0; i < num_med_laser_act_ && num_med_util < cNumMedLaserNorm; ++i)
  {
    ...
    if (prob_ocupacion_a <= umbral_no_ocupacion_ && prob_ocupacion_b <= umbral_no_ocupacion_)
    {
      // cout << "CDO2 -- MA: UB_LLEG_G LIBRE -- MB: UB_LLEG_G LIBRE.\n";
      trazadoRayos2(ub_g, incr_x_ex_g, incr_y_ex_g, dist_a[i], dist_b[i]);
    }
    else if (prob_ocupacion_a <= umbral_no_ocupacion_ && prob_ocupacion_b >= umbral_ocupacion_)
    {
      // cout << "CDO2 -- MA: UB_LLEG_G LIBRE -- MB: UB_LLEG_G OCUPADA.\n";
      dist_a[i] = trazadoRayos(mapa_a_, ub_g, incr_x_ex_g, incr_y_ex_g, 0);
    }
    else
    {
      // cout << "CDO2 -- MA: UB_LLEG_G OCUPADA -- MB: UB_LLEG_G LIBRE.\n";
...
// cout << "\n";
// cout << "factor_atenuacion_ -- COSTE: " << coste << "\n";
  ...
  if (bandera_loc_global_exito_)
  {
    ...
    coste += (coef_hipotenusa_ * (lado_celda_M_ * hypot(incr_x_g, incr_y_g))) + (coef_angulo_ * fabs(incr_t_g));
  }
...
std::vector<std::vector<unsigned int> > LocED::crearPermutaciones()
{
  ...
  std::vector<std::vector<unsigned int> > rotaciones(numero_componentes_,
                                                     std::vector<unsigned int>(num_elem_pob_act_, 0));
...
// Individuos ancestros.
  ...
  do
  {
    ...
    for (i = 0; i < num_elem_pob_act_; ++i)
    {
      ...
      // MUTACION
      x_mut_g = x_r0_g + (factor_atenuacion_ * (x_r1_g - x_r2_g));
      y_mut_g = y_r0_g + (factor_atenuacion_ * (y_r1_g - y_r2_g));
      t_mut_g = t_r0_g + (factor_atenuacion_ * (t_r1_g - t_r2_g));
      ...
      // cout << "\n");
      // cout << "CSM -- I: %d.\n", i);
      // cout << "CSM -- X_CRUCE_G: %.8f.\n", x_cruce_g);
      // cout << "CSM -- Y_CRUCE_G: %.8f.\n", y_cruce_g);
      // cout << "CSM -- T_CRUCE_G: %.8f.\n", scGradRad * t_cruce_g);
...
// SE PINTAN TODAS LAS MEDIDAS DEL SENSOR LASER REAL.
  for (i = 0; i < cNumMedLaser; ++i)
  {
    ...
    // cout << "CEX = %f\n", (dist_laser[i] * cos(t_ex_g)) / ladoCeldaM_);
    // cout << "CEY = %f\n", (dist_laser[i] * sin(t_ex_g)) / ladoCeldaM_);
    x_lleg_g = x_est_g + ((dist_laser[i] * cos(t_ex_g)) / lado_celda_M_);
    y_lleg_g = y_est_g + ((dist_laser[i] * sin(t_ex_g)) / lado_celda_M_);
...
//cout << "\n";
//cout << "DP - num_elem_pob: " << num_elem_pob << "\n";
...
void LocED::obtenerMapa(const nav_msgs::OccupancyGrid::ConstPtr& rp_msj_rejilla_ocupacion_rx)
{
  ...
  std::vector<double> fila(num_columnas, 0);
  std::vector<std::vector<double> > matriz_ocupacion(num_filas, fila);
  unsigned int inicio = 0;

  cout << "Filas: " << num_filas << endl;
  cout << "Columnas: " << num_columnas << endl;

  // Si un mapa tiene N filas, desde la 0 a la N-1, y M col
  // en ROS se trasmite primero la fila N-1, luego la N-2, asi hasta la 0.
  // Ejemplo: [fila_N-1(col0, col1, col2, ...colM-1), fila_N-2(col0, col1, col2, ...colM-1), ... fila_0(col0, col1, col2, ...colM-1)].
  for (int i = num_filas - 1; i >= 0; i--)
  {
    ...
    for (int j = 0; j < num_columnas; j++)
    {
      ...
      fila[j] = (rp_msj_rejilla_ocupacion_rx->data[inicio + j] / 100.0);
      //cout << "\t[" << j << "]   " << fila[j] << endl;
    }
    inicio += num_columnas;
    matriz_ocupacion[i].assign(fila.begin(), fila.end());
  }

  lado_celda_M_ = rp_msj_rejilla_ocupacion_rx->info.resolution;
  dist_traslacion_M_ = lado_celda_M_ * dist_traslacion_C_;
  dist_convergencia_C_ = dist_convergencia_M_ / lado_celda_M_;
  alcance_laser_C_ = alcance_laser_M_ / lado_celda_M_;
  coef_hipotenusa_ = 1.0 * lado_celda_M_ / cDist3SigmaM;
  coef_angulo_ = 10 * lado_celda_M_ / cDist3SigmaM;

  ub_max_regint_g_.setXYTheta(num_columnas, num_filas, 2 * M_PI);

  mapa_a_.setFilas(num_filas);
  mapa_a_.setColumnas(num_columnas);
  mapa_a_.setMatrizProbabilidadOcupacion(matriz_ocupacion);

  cout << "lado_celda_M_: " << lado_celda_M_ << endl;
  cout << "distTraslacionM: " << dist_traslacion_M_ << endl;
  cout << "dist_convergencia_C_: " << dist_convergencia_C_ << endl;
  cout << "alcance_laser_C_: " << alcance_laser_C_ << endl;
  cout << "coef_hipotenusa_: " << coef_hipotenusa_ << endl;
  cout << "coef_angulo_: " << coef_angulo_ << endl;

  cout << "x_min_regint_g: " << ub_min_regint_g_.getX() << endl;
  cout << "y_min_regint_g: " << ub_min_regint_g_.getY() << endl;
  cout << "t_min_regint_g: " << scGradRad * ub_min_regint_g_.getTheta() << endl;
  cout << "x_max_regint_g: " << ub_max_regint_g_.getX() << endl;
  cout << "y_max_regint_g: " << ub_max_regint_g_.getY() << endl;
  cout << "t_max_regint_g: " << scGradRad * ub_max_regint_g_.getTheta() << endl;
  ...
  /*
   ...
   {
   cout << "[ " << mapa_a_.obtenerProbabilidadOcupacion(j, i) << " " << matriz_ocupacion[i][j] << "]  ";
   }
   cout << endl;
   }
   */
...
// cout << "\n";
// cout << "CDO -- UB_G: (" << x_g << ", " << y_g << ", " << scGradRad * ub_g.getTheta() << ").\n";
// cout << "CDO -- UB_LLEG_G: (" << x_lleg_g << ", " << y_lleg_g << ", " << scGradRad * t_ex_g << ").\n";
// cout << "CDO -- INC_X_EX_G: " << incr_x_ex_g << ".\n";
// cout << "CDO -- INC_Y_EX_G: " << incr_y_ex_g << ".\n";
...
// 3.- Se excedan los limites del mapa.
  do
  {
    ...
    dist += dist_traslacion_M_;
    //cout << "\n";
    //cout <<"CDO2 -- UB_LLEG_G: (" x_lleg_g << ", " << y_lleg_g << ", " << scGradRad * t_ex_g << ").\n";
    ...
    if (x_lleg_g < x_min_mapa || x_lleg_g >= x_max_mapa || y_lleg_g < y_min_mapa || y_lleg_g >= y_max_mapa)
    {
      ...
      // while(prob_ocupacion <= umbral_no_ocupacion_), este se detenga porque la condicion
      // no se satisface.
      prob_ocupacion = 1;
      continue;
    }
    ...
    roundLoc(x_lleg_g, y_lleg_g, c_x_lleg_g, f_y_lleg_g);
    prob_ocupacion = mapa.obtenerProbabilidadOcupacion(c_x_lleg_g, f_y_lleg_g);

    // cout << "CDO -- UB_LLEG_G (ROUND): (" << c_x_lleg_g << ", " << f_y_lleg_g << ", " << scGradRad * t_ex_g << ").\n";
    // cout << "CDO -- PROB: " << prob_ocupacion << "\n";

  } while (dist < alcance_laser_M_ && prob_ocupacion <= umbral_no_ocupacion_);

// cout << "\n";
// cout << "CDO -- UB_G: (" << x_g << ", " << y_g << ", " << scGradRad * t_ex_g << ").\n";
// cout << "CDO -- UB_LLEG_G: (" << x_lleg_g << ", " << y_lleg_g << ", " << scGradRad * t_ex_g << ").\n";
// cout << "CDO -- DIST: " << dist << ".\n";
...
// cout << "\n";
// cout << "CDO2 -- UB_G: (" << x_g << ", " << y_g << ", " <<  scGradRad * ub_g.getTheta() << ").\n";
// cout << "CDO2 -- UB_LLEG_G: (" << x_lleg_g << ", " << y_lleg_g << ", " << scGradRad * t_ex_g << ").\n";
// cout << "CDO2 -- INC_X_EX_G: " << incr_x_ex_g << ".\n";
// cout << "CDO2 -- INC_Y_EX_G: " << incr_y_ex_g << ".\n";
...
// 3.- Se excedan los limites del mapa.
  ...
  do
  {
    ...
    {
      // cout << "\n";
      // cout << "CDO2 -- UB_LLEG_G FUERA MAPA.\n";
      ...
      // la iteracion actual del bucle, y me lleve a la expresion while(prob_pos_libre <= umbral_no_ocupacion_),
    ...
    roundLoc(x_lleg_g, y_lleg_g, c_x_lleg_g, f_y_lleg_g);
    prob_ocupacion_a = mapa_a_.obtenerProbabilidadOcupacion(c_x_lleg_g, f_y_lleg_g);
    prob_ocupacion_b = mapa_b_.obtenerProbabilidadOcupacion(c_x_lleg_g, f_y_lleg_g);

    // cout << "CDO2-- UB_LLEG_G (ROUND): (" << c_x_lleg_g << ", " << f_y_lleg_g << ", " << scGradRad * t_ex_g << ").\n";
    // cout << "CDO2 -- PROB_A: " << prob_ocupacion_a << "\n";
    // cout << "CDO2 -- PROB_B: " << prob_ocupacion_b << "\n";
    ...
    // es capaz de medir, o si alguna de las probabilidades hayadas es superior a
    // 'umbral_no_ocupacion_'
  }while (dist < alcance_laser_M_ && prob_ocupacion_a <= umbral_no_ocupacion_ && prob_ocupacion_b <= umbral_no_ocupacion_);

// cout << "\n";
// cout << "CDO2 -- UB_G: (" << x_g << ", " << y_g << ", " << scGradRad * t_ex_g << ").\n";
// cout << "CDO2 -- UB_LLEG_G: (" << x_lleg_g << ", " << y_lleg_g << ", " << scGradRad * t_ex_g << ").\n";
// cout << "CDO2 -- DIST: " << dist << ".\n";
...
// comprobaciones y se gana velocidad de ejecucion.
  ...
  {
    // cout << "\n";
    // cout << "CDO2 -- DIST MAX ALCANZADA: " << cAlcanceLaserM << ".\n";
...
// Obstaculo encontrado en AMBOS mapas.
  ...
  else if (prob_ocupacion_a >= umbral_ocupacion_ && prob_ocupacion_b >= umbral_ocupacion_)
  {
    // cout << "\n";
    // cout << "CDO2 -- MA y MB: UB_LLEG_G OCUPADA.\n";
...
// else if (prob_ocupacion_a >= umbral_ocupacion_ && prob_ocupacion_b <= umbral_no_ocupacion_)
...
// cout << "\n";
// cout << "CDO -- DIST_A: " << dist_a[i] << ".\n";
// cout << "CDO -- DIST_B: " << dist_b[i] << ".\n";
