...
LocED::LocED(const unsigned int& d, const unsigned int& st, const float& cr, const float& f, const float& t_c,
             ...
             const std::string& nomb_fich_b, const float& alcance_laser_m, const double& angulo_max,
    ...
    numero_componentes_(d), estrategia_mutacion_(st), factor_cruce_(cr), factor_atenuacion_(f), dist_traslacion_C_(t_c), dist_traslacion_M_(
        ...
        num_iter_norm), num_iter_conv_(num_iter_conv), num_iter_act_(num_iter_norm), mapa_recibido_(0), aptitudUbEstSrg_(
        0), numLocSinExito_(0), alcance_laser_M_(alcance_laser_m), alcance_laser_C_(0), cAnguloMax(
    ...
    cNumMedLaserNorm(2 * ((unsigned int)(angulo_max / (coef_incr * res_angular))) + 1), cNumMedLaserConv(
        ...
        scCoefAptitudLimSup * (2 * cDist3SigmaM * num_med_laser_act_)), coef_hipotenusa_(0), coef_angulo_(0), intervaloTiempoSegLocSinExito_(
        0)
{
  ...
  cout << "prob_libre_: " << umbral_ocupacion_ << endl;
  cout << "banderaLocConExito_: " << bandera_loc_global_exito_ << endl;
...
// 3.- Se excedan los limites del mapa.
  ...
  do
  {
    ...
    // cout << "CDO2 -- PROB_A: " << prob_p_libre_a << "\n";
    // cout << "CDO2 -- PROB_B: " << prob_p_libre_b << "\n";
