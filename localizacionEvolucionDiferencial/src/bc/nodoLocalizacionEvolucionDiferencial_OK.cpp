...
int main(int argc, char* argv[])
{
  ...
  LocED loced(tamanioCeldaM, cEvDifD, cEvDifST, cEvDifCR, cEvDifF, cDistanciaTraslacionC, cDistConvM, cNumElemPobNorm, cNumElemPobConv,
              cNumIterNorm, cNumIterConv, nomb_fich_a, nomb_fich_b, ub_min_regint_g, ub_max_regint_g, cAlcanceLaserM,
  ...
  // Desde la ubicacion de interes el robot real toma un barrido laser e informa del valor de sus coordenadas en el
  // sistema de referencia odometrico.
  ...
  // La poblacion se crea en la region de interes.
  ...
  do
  {
    ...
    cout << "\x1B[32;1m";
    cout << "x_est_rr_srg: " << tamanioCeldaM * ub.getX() << " m " << endl;
    cout << "y_est_rr_srg: " << tamanioCeldaM * (loced.getNumeroFilasMapa() - ub.getY()) << " m " << endl;
    cout << "t_est_rr_srg: " << LocED::scGradRad * anguloSRG << " grad " << endl;
