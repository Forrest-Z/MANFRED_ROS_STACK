...
public:
  ...
  inline static void orientacionMenosPiMasPi(double& anguloRad){

	  double dosPI = 2 * M_PI;
	  double signoThetaDosPI = (anguloRad < 0 ? -1 : anguloRad > 0) * dosPI;
	  if (fabs(anguloRad) > dosPI)
	  {
	    ...
	    anguloRad = fmod(anguloRad, signoThetaDosPI);
	  }
	  // Finalmente hacer que el valor de deltaTheta este entre -pi y pi.
	  if (fabs(anguloRad) > M_PI)
	  {
	    anguloRad -= signoThetaDosPI;
	  }
  }
